	...
	<form name="popForm" method="get">
			...
			<div id="view">
				<table id="searchItem" style="width: 100%;">
					...
					<c:forEach var="ow" items="${outwareView}">
						<tr align="center">
							<td><a
								href="javascript:popFunction('${ow.outsourcing_Code }','${ow.outsourcing_Name }')">${ow.outsourcing_Code }</a></td>
							<td><a
								href="javascript:popFunction('${ow.outsourcing_Code }','${ow.outsourcing_Name }')">${ow.outsourcing_Name }</a></td>
						</tr>
					</c:forEach>
		...
		<script>
			...
			submit_button.onclick = function() { //적용버튼에 온클릭이벤트 부여함 (팝업창)
				$(opener.document).find("#outsourcingCode").val($("#code").val()); //find()는 body.jsp의 텍스트박스의값임, val()는 팝업.jsp의 텍스트박스의값임
				$(opener.document).find("#outsourcingName")
						.val($("#name").val());
				window.close();
			}
